...
void geom_brownian_vector(double* x,double x0, double mu, double sigma, double dt,
     size_t n,gsl_rng* rng)
{
    ...
    for(size_t i = 1; i < n; ++i){
        ...
        double prev = x[i-1];
        x[i] = prev + prev*drift + sigma*prev*dW;
    }
}
void geom_brownian_matrix(double* A,double x0, double mu, double sigma, double dt, 
    size_t n,size_t m,gsl_rng* rng)
{
    // Generates a matrix of m geometric brownian vectors, each of length n.
    ...
    for(size_t j = 0; j < m; ++j){
        ...
        for(size_t i = 1; i < n; ++i){
                double dW = gsl_ran_gaussian(rng,sqrt_dt);
                double prev = A[i-1+n*j];
                A[i+n*j] = prev + prev*drift + sigma*prev*dW;
        }
    }
}
...
{
    // Simulates m independent geometric brownian processes, 
